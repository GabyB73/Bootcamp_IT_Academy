...
public class PasarelaDePago {
    ...
    void realizarPago(MetodoDePago metodoDePago, CallbackPago callbackPago) {
        ...
        boolean pagoExitoso = Math.random() < 0.8; // Simula el 80% de probabilidades de éxito

        // Llamar al Callback para notificar el resultado del pago
        if (pagoExitoso) {
            callbackPago.pagoExitoso(metodoDePago.getTipoDePago());
        } else {
            callbackPago.pagoFallido(metodoDePago.getTipoDePago());
        }
    }
}
